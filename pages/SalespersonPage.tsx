...
import { Briefcase, List, GitMerge } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 relative">
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
           <div className="flex items-center gap-2 mb-2">
               <span className="bg-indigo-100 text-indigo-800 text-xs px-2 py-0.5 rounded-full font-bold uppercase tracking-wide flex items-center gap-1">
        ...
        <div className="flex items-center gap-3">
          <div className="bg-white p-1 rounded-lg border border-slate-200 shadow-sm flex">
             <button 
               ...
               className={`px-3 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 transition-colors ${viewMode === 'list' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:text-slate-700'}`}
             >
               <List className="w-4 h-4" /> List
             </button>
             <button 
               onClick={() => setViewMode('diagram')}
               className={`px-3 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 transition-colors ${viewMode === 'diagram' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:text-slate-700'}`}
             >
      ...
      {viewMode === 'list' ? (
        ...
        <WorkflowDiagram phases={SALESPERSON_WORKFLOW_PHASES} title="Salesperson Process Map" />
      )}
    </div>
  );
};
